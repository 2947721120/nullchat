...
(function(){
  ...
  angular
	...
	.config(['$mdThemingProvider', '$mdIconProvider', '$urlRouterProvider', '$stateProvider', '$analyticsProvider', function($mdThemingProvider, $mdIconProvider, $urlRouterProvider, $stateProvider, $analyticsProvider){
		...
		$mdThemingProvider.theme('default')//$ MD主题化提供商主题（“默认”）
		  .primaryPalette('blue')//.primary调色板（“蓝色”）
		  .accentPalette('teal');//.accent调色板（“水鸭”）
...
(function(){
  ...
  function NCController( NCService, $q, $scope, $sce, $rootScope, $window, $mdSidenav, $stateParams, $mdDialog) {
    ...
    self.showNameDialog = showNameDialog;//聊天室名称对话框
    ...
    var idle = new $window.Idle();//自动怠速=新的$ window.Idle（）;
    ...
    idle.setAwayTimeout(5000);//idle.设置客场超时（5000）
    idle.start();
    ...
    function showNameDialog(ev) {
      $mdDialog.show({
        targetEvent: ev,//目标事件：ev
        templateUrl : "views/nameDialog.html",//模板网址
        clickOutsideToClose : true,//
        preserveScope : true,//保存范围
        bindToController : true,//绑定控制器
  ...
  function UserNameChangeController(NCService, $mdDialog, $rootScope) {
    ...
    self.authUser = authUser;//自动用户
    self.authState = NCService.getAuthState('github');//github登陆
    self.logoutUser = logoutUser;//注销用户=注销用户;
    self.showProfileImage = NCService.getUserSettings('showProfileImage');//nc服务.获得用户设置图片
    self.saveSettings = saveSettings;//保存设置
    console.log(self.authState);//控制台日志（个体经营自动状态）
    function saveSettings() {//功能 保存设置
    }
    function closeDialog() {//功能关闭对话框
      $mdDialog.cancel(); //取消
    }
    function changeUserName() {//功能更改用户名（）{
      NCService.changeUserName(self.userName);//NC服务更改用户名（自己.用户名）
      $mdDialog.cancel(); //取消
    }
    function logoutUser() {//用户退出
      NCService.logoutUser().then(function() {//NC服务注销用户（）。然后（函数（）
        NCService.goHome();//服务.去首页
        $mdDialog.cancel(); //关闭对话框
      });
    }
    function authUser(loginProvider) {//函数 自动用户 登陆 供应商
          NCService.authUser(loginProvider).then(function() {//服务.自动用户(登陆供应商).然后(功能()
            NCService.goHome();//服务.去首页
  ...
  function ChatBoxController(NCService, $timeout, $mdDialog, $stateParams, $rootScope, $q, chatValid, $state, $location, $window) {
      var self = this;
      self.chatId = $stateParams.chatId;//自己.聊天id=$状态参数.聊天id
      self.uploadProgress = 0;//上传进度
      if(chatValid === false) {//如果(聊天有效==假)
        NCService.leaveChat(self.chatId);
        $state.transitionTo('dashboard');
      }
      $rootScope.loadingActivity = false;//加载活动
      ...
      self.handleKeyDown = handleKeyDown;//处理键
      ...
      self.holderThemes = [ 'sky', 'vine', 'lava', 'industrial', 'social' ];//自持有人主题= ['天空'，'藤'，'熔岩'，'产业'，'社会'
      ...
      function renameChat() {//重命名聊天室
        $mdDialog.show({//对话框.显示
          targetEvent: self.menuOriginatorEv,//目标事件：自创始菜单EV
          clickOutsideToClose: true,//点击触发关闭
          preserveScope : true,//保护范围
          bindToController : true,//绑定到控制器
          templateUrl : "views/chatDialog.html",//模板
          controller: 'ChatDialogController',//聊天对话框控制器
          controllerAs : 'cd',//控制器as:'cd' 
       });
      }
      function resetUnreadCount(chatId) {//未读计数复位
        NCService.resetUnreadCount(self.chatId);//服务未读计数复位(聊天ID)
      }
      function removeFocus() {//删除焦点
      ...
      function uploadFile(file) {//上传文件（文件）
        ...
        NCService.uploadFile(file).then(//NC服务上传文件（文件）,然后
          ...
          function(resp) {
            console.log(resp);//控制台日志
            alert('图片上传失败');//警报（'图片上传失败'）;
      ...
      function showShareModal() {//功能 显示分享模态
        alert = $mdDialog.alert({//警报= 对话框.警报
          title: '分享',//标题'分享'
          textContent: $location.absUrl(),//$本地.abs网址()
          ok: '确定'
        });
        $mdDialog
          .show( alert )//显示(警报)
      ...
      function loadUserInfo() {//加载用户信息
        NCService.loadUserInfo(self.chatId);//服务.加载用户信息(自己.聊天Id)
      }
      function leaveChat() {//离开聊天室
        NCService.leaveChat(self.chatId);//服务.离开聊天
      }
      function Speech() {
        var recognition = new webkitSpeechRecognition();
        }
      function sendChat() {//发送聊天
        if(self.chatText) {//如果(自己.聊天文本)
          NCService.appendArray(self.chatId, 'text', self.chatText);//服务.附加阵列(自己.聊天id,'文本',自己.聊天文本)
          self.chatText = ""; //自己聊天文本=''
          self.changeTypingStatus(self.chatId, 'remove');//自己.更改输入状态（自聊天的身份证，“删除”
        }
      }
      function handleKeyDown(e) {//操作键(e)
        var self = this;
      ...
      function showOwner(uid) {//显示业主
        return NCService.showOwner(self.chatId, uid);//返回 服务.显示 业主 (自己.聊天,uid)
      }
      function changeTypingStatus() {//变化 输入状态
        var action = 'add';//输出 行动='添加'
        if(!self.chatText) {//如果不等于自己.聊天文本
          action = 'remove';//行动删除
        }
        NCService.changeTypingStatus(self.chatId, action);//服务改变输入状态(自己.聊天id,行动)
      }
      function showTypingStatus() {//显示输入状态
        return NCService.showTypingStatus(self.chatId);//返回服务.显示输入状态(自己.聊天Id)
      }
  }
...
(function(){
  ...
  function NCService($q, $analytics, $firebaseArray, $firebaseObject, $firebaseAuth, $rootScope, $stateParams, $state, $mdToast, Upload){
    var states = { INIT: 0, RUNNING : 1, LOADED: 2 };//输出状态 初始化=0 运行=1 加载=2
    // 基于承诺的API
    return {
      syncObject : { user : null, users : {} , chats : {} },//同步对象:(用户:空, 复数用户:{}, 聊天{}}
      uid : 0,//
      fbRef : null,//读fb
      authObj: null,//自动对象
      chats : [],// 聊天
      ...
      makeid : function(length) {//
          var text = "";//输出 文本
          var possible = "0123456789";//用户名及聊天室自动生成筛选文本
          for( var i=0; i < length; i++ )//对于(输出 i=0
              text += possible.charAt(Math.floor(Math.random() * possible.length));//文字+ = possible char在（数学楼（数学。（）*的可能。长度））
          return text;
      },
      ...
      init : function(bindObj) {//初始化绑定OBJ
        ...
        var def = $q.defer();
        //初始化状态
        if($rootScope.currentState === undefined) {//根
          self.initRootScope();
        ...
        if($rootScope.currentState == states.INIT) {
          ...
          self.fbRef = new Firebase("https://c2cmall.firebaseio.com/");
          ...
          self.authObj.$waitForAuth().then(function(authData) {
            if(authData && authData.expires) {
              var d = new Date();////新事件
              var n = d.getTime();//获得时间
              var sessionTimeout = authData.expires*1000;//缓存时间退出
              var remainingTime = (authData.expires*1000 - n)/60000;
            }
            if(!authData || remainingTime < 1) {
      ...
      changeUserName : function(name) {
        name = name.trim();
        if(name && name.toUpperCase() != '我' //如果(名称 && 名称.至大写字母()不等于'我'
            && name.toUpperCase() != 'ADMIN' //名称.至大写字母不等于'管理员'
            && name.toUpperCase().indexOf('FUCK') === -1) {
          $rootScope.user.meta.name =  name ;//根范围.用户.元.名称=名称
        }
      },
      initStructUser : function() {//初始化结构体用户 功能
        ...
        if($rootScope.user.meta === undefined) {//如果元根用户范围===元未定义
            ...
            self.changeUserName("用户" + self.makeid(5));//自己.更改用户名（“用户”+自主制造ID(5位))
        }
        if($rootScope.user.auth === undefined) {//如果根范围用户.自动 等于 未定义
            $rootScope.user.auth = {};//根范围.用户.自动={}
        }
        if($rootScope.user.settings === undefined) {//如果用户设置等于未定义
            $rootScope.user.settings = {};//根范围.用户设置={}
        }
        if($rootScope.user.chats === undefined) {//如果(根范围.用户.聊天等于未定义)
      ...
      getUser : function() {//获得用户:函数
        var self = this;
        if(self.isNcdataInit()) {//如果自己是数据初始化
          return { name: $rootScope.user.meta.name };//返回(名称:根范围.用户.元.名称)
        } else {
          return { name: '' };//返回{名称:''}
        }
      },
      createChat : function() {//创建聊天室
        var self = this;
        var chatId = this.makeid(5);//聊天室ID等于自动生成5个字符串
        if($rootScope.user.chats.own === undefined) {//如果根用户聊天拥有===未定义
         $rootScope.user.chats.own = [];//根范围.用户.lt .拥有
        }
          ...
          if($rootScope.chats[chatId].data.typing !== undefined) {
            if(Object.keys($rootScope.chats[chatId].data.typing).length > 0 ) {
              ...
              angular.forEach($rootScope.chats[chatId].data.typing, function(timestamp, uid){
                if(uid !== self.uid && (self.getCurTimestamp() - timestamp < 10000) ) {//如果（UID==自我UID&&（个体经营得到姜黄素时间戳（） - 时间戳<10000）
                  if(self.showOwner(chatId, uid))//如果（自显示所有者（聊天标识UID））//
      ...
      addUserToChat : function(chatId) {//将用户添加到聊天：功能（聊天标识
        var self = this;
        var isJoined = false;
        //如果当前用户不拥有chatId然后将其添加到用户的共享 "shared" 列表
        // ... 并将其添加到 "分享" 阵列中的聊天数据。
        if($rootScope.chats[chatId].meta.owner !== self.uid) {
          if($rootScope.user.chats.shared === undefined) {//如果聊天ID数据分享未定义
           $rootScope.user.chats.shared = [];//用户分享=没有
          }
        ...
        //加上本人或共享的家伙到列表
        if($rootScope.chats[chatId].data.sharedWith === undefined) {//如果聊天ID数据分享未定义
      ...
      isInit : function() {
        ...
        var removeCheck = $rootScope.$watch("currentState", function(currentState) {//VAR删除检查= $根范围。$表（“当前状态”功能（当前状态）
          if(currentState == states.LOADED) {//目前状态==装载状态
            removeCheck();
            def.resolve();
          }
        });
      ...
      loadUserInfo : function(chatId) {//加载用户信息
        ...
        angular.forEach(sharedWith, function(value, uid){//角。为每一个（共享带，功能（价值，UID）
          if($rootScope.users[uid] === undefined) {//如果（$根Scope.users[UID]===未定义）
            ...
            self.syncObject.users[uid] = $firebaseObject(self.fbRef.child('users').child(uid).child('meta'));//自己.同步 对象.用户[UID] = $火力对象（self.fb Ref.child（'用户'）。子女（UID）.child（“元”））
      ...
      leaveChat : function(chatId) {//离开聊天：功能（聊天标识）{
        ...
        if(isChatOwner) {
          ...
          $mdToast.show($mdToast.simple().position('top right').textContent('你是这个不再聊天中'));
          $state.transitionTo('dashboard');//状态.转型('仪表台')
        }
      },
      playNotification : function() {//播放通知：功能()
        /*增加桌面通知开始*/
  var title = "在线聊天";//增加桌面通知标题
  var options = {//选项
      body: "您有新信息",
      icon: "https://cdn.c2cmalls.com/nullchat/32.png"
  };
  // 让我们来看看浏览器是否支持通知
  ...
  // 让我们来看看是否通知权限已被授予
  else if (Notification.permission === "granted") {// 如果这是可以让我们创建一个通知
    var notification = new Notification(title, options);//输出通知=新 通知包含(标题,选项)
    notification.onshow = function() {
      ...
      initChatData : function(chatId, loadMore) {//初始化字符数据：功能（chatId，加载更多
        var self = this;
        var arrayLength = 100;//VAR数组长度=200
        if(loadMore) {
  ...
  self.appendArray(chatId, 'status', '加入');//自追加数组（聊天身份证，“状态”，“加入'）;
      ...
      loadChatData : function(chatId) {//加载聊天数据
        var self = this;
        var def = $q.defer();
        if($rootScope.chats[chatId] !== undefined && $rootScope.chats[chatId].data !== undefined) {//如果id不=未定义
          $rootScope.chats[chatId].unreadCount = 0;//聊天ID.未读信息=0
          def.resolve(true);
          return def.promise;
        }
       //加载数据
      ...
      isChatOwner: function(chatId) {
        ...
        if(self.isNcdataInit() && self.isChatLoaded(chatId) && $rootScope.chats[chatId].meta.owner) {//如果（self.是 NC数据的init（）&& self.是加载聊天（聊天标识）&&$根Scope.chats[聊天ID].META所有者）
          return $rootScope.chats[chatId].meta.owner === self.uid;//返回 根范围 .聊天 [聊天id].元.主人
        } else {
          return false;
        }
      },
      ...
      authUser : function(loginProvider) {
        ...
        var self = this;
        var def = $q.defer();
        self.fbRef.authWithOAuthPopup("github", function(error, authData) {
          if (error) {
            console.log("登录失败!", error);
            def.reject();
          } else {
