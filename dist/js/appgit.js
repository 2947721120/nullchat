...
(function(){
  ...
  function NCController( NCService, $q, $scope, $sce, $rootScope, $window, $mdSidenav, $stateParams, $mdDialog) {
    ...
    var idle = new $window.Idle();//汽车怠速=新的$ window.Idle（）;
  ...
  function UserNameChangeController(NCService, $mdDialog, $rootScope) {
    ...
    self.authState = NCService.getAuthState('github');
    ...
    function changeUserName() {//功能更改用户名（）{
      NCService.changeUserName(self.userName);//NC服务更改用户名（个体经营用户名）
      $mdDialog.cancel(); //取消
    }
  ...
  function ChatBoxController(NCService, $timeout, $mdDialog, $stateParams, $rootScope, $q, chatValid, $state, $location, $window) {
      ...
      self.holderThemes = [ 'sky', 'vine', 'lava', 'industrial', 'social' ];//自己头像主题色= ['天空'，'藤'，'熔岩'，'产业'，'社会'
  ...
  self.appendArray(chatId, 'status', '加入');//自追加数组（聊天身份证，“状态”，“加入'）;
      ...
      uploadFile : function(file) {
        ...
        return Upload.http({
         ...
         Authorization : ['CLIENT_ID：16bf0d67fef29ba', 'client_secret：ebf8f144cc1282cca05a5e62ab66132d439be670 '],
            },
            data : file
          });
      }
